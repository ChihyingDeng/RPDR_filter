...
def reformat_dates(timestamp_string):
    """
    Return a datetime corresponding to the timestamp string. If the format is not accepted, return None.
    """
    fmts = (
        '%m/%d/%Y',
        '%Y/%m/%d',
    ...
    for fmt in fmts:
        try:
            date = datetime.strptime(timestamp_string, fmt)
            break
...
def get_empi_to_date_range(filter_fname, patient, date, include, days_before, days_after):
    """
    map patient ID to a list of dictionary {start, end, include}
    Args:
        filter_fname: [str]  filter file name
        patient:      [str]  column name for patient ID
        date:         [str]  column name for procedure date
        include:      [str]  whether include this patient within a specified time range
        days_before:  [int]  how many days before the procedure
        days_after:   [int]  how many days after the procedue
    Output:
        [dict] dictonary of filtering criteria (specified time ranges for each patient)
    """
    empi_to_date_range = collections.defaultdict(list)

    if include == 'All':
        filter_df = pd.read_csv(filter_fname, usecols=[
                                patient, date])
    else:
        filter_df = pd.read_csv(filter_fname, usecols=[
...
def filter_rpdr_file(input_fname, empi_to_date_range, days_before, days_after):
    """
    filter rpdr file using the criteria from filter file and convert to CSV file after filtering
    Args:
        input_fname:        [str]  input RPDR file name
        empi_to_date_range: [dict] dictonary of filtering criteria (specified time ranges for each patient)
        days_before:        [int]  how many days before the procedure
        days_after:         [int]  how many days after the procedue
    """
